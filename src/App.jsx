...
const LiveParticipant = ({ part, patchVersion }) => {
  return (
    <div className={`flex items-center gap-3 p-2 rounded-xl border transition-colors ${part.isTrackedPlayer ? 'bg-[#ffb800]/10 border-[#ffb800]/30' : 'bg-white/5 border-transparent'}`}>
      <img 
      ...
      <div className="flex flex-col min-w-0 flex-1">
        <span className={`text-sm font-bold truncate uppercase tracking-tight ${part.isTrackedPlayer ? 'text-[#ffb800]' : 'text-slate-200'}`}>
          {part.summonerName}
        </span>
...
export default function App() {
  ...
  const displayedPlayers = roster.filter(player => player.category === activeTab);
        ...
        {displayedPlayers.map((player) => {
          ...
          return (
            ...
            >
              ...
              <div className={`relative w-20 h-20 rounded-full flex items-center justify-center mb-5 flex-shrink-0 ${isPlaying ? 'shadow-[0_0_30px_rgba(200,170,110,0.5)]' : 'grayscale opacity-50 group-hover:opacity-100 group-hover:grayscale-0'} transition-all duration-500`}>
                  <div className={`absolute -inset-1.5 rounded-full border-2 ${isPlaying ? 'border-[#c8aa6e]' : 'border-[#1e2328]'} opacity-30`}></div>
              ...
              <div className="text-center z-10 w-full">
                <h2 className={`text-base font-black uppercase tracking-wider truncate text-white leading-tight`}>{player.name}</h2>
                <span className="text-[10px] text-[#c8aa6e] font-bold uppercase tracking-[0.2em] opacity-50 mb-3 block">#{player.tag}</span>
                ...
                <p className="text-[10px] text-slate-500 mt-2 font-medium">
                  {isPlaying ? `${liveData.time}m elapsed` : isOffline ? `Offline` : `Waiting for engine...`}
                </p>
              </div>
            </div>
          );
          ...
          <div className="fixed inset-0 bg-[#030408]/90 backdrop-blur-xl flex items-center justify-center z-50 p-4" onClick={() => setSelectedPlayer(null)}>
            <div className="bg-[#0d1016] border border-[#1e2328] inner-glow-cyan rounded-3xl p-8 md:p-10 w-full max-w-4xl shadow-2xl relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <div className="mb-10 text-center flex flex-col items-center mt-4">
                ...
                <h3 className="text-3xl font-black text-white italic uppercase tracking-tighter">{selectedPlayer.name} <span className="text-cyan-400">MATCH</span></h3>
                <p className="text-slate-400 text-xs font-bold mt-1 tracking-widest uppercase">EN CURSO • {matchInfo.time} MINUTES</p>
              </div>
